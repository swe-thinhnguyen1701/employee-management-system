...
//   ]);
//   const managerID =
//     res.managerName !== "None"
//       ? managerList.indexOf(res.managerName) + 1
//       : null;
//   console.log([
//     `'${res.firstName}'`,
//     `'${res.lastName}'`,
//     roleList.indexOf(res.roleTitle) + 1,
//     managerID,
//   ]);
//   return [
//     res.firstName,
...
const updateEmployeeRole = async (id) => {
  ...
  return [
    [res.employeeName, res.newRole],
    [
      roleList.indexOf(res.newRole) + 1,
      employeeList.indexOf(res.employeeName) + 1,
    ],
  ];
};
...
const driver = async () => {
  ...
  try {
    while (isRunning) {
      const dataService = await prompt.mainMenuPrompt();
      if(dataService.addNewData){
        const values = await addNewDataPrompt(dataService.id);
        const message = await crud.addData(dataService.id, values);
      }
    }
    console.log(id);
  } catch (error) {
